...
import type { Volunteer, VolunteerFormState } from "../types";
...
export function VolunteerForm({
  ...
  initial?: Volunteer;
  action: (state: VolunteerFormState, formData: FormData) => Promise<VolunteerFormState>;
}) {
  ...
  return (
    <form action={formAction} className="flex max-w-xl flex-col gap-4">
      ...
      <label className="flex flex-col gap-1 text-sm">
        Color tag
        <select name="colorTag" defaultValue={initial?.colorTag ?? "green"} className="border p-2">
